...
const isHeic = bytes => {
	// Check for 'ftyp' box at bytes 4-7 and HEIC/HEIF brand
	if (bytes.length < 12) {
		return false;
	}

	// 'ftyp' magic bytes at offset 4
	const hasValidFtyp = bytes[4] === 0x66 // F
		&& bytes[5] === 0x74 // T
		&& bytes[6] === 0x79 // Y
		&& bytes[7] === 0x70; // P
	...
	// Check for HEIC/HEIF brand at offset 8-11
	const brand = String.fromCodePoint(...bytes.slice(8, 12));
	return brand === 'heic' || brand === 'heix' || brand === 'hevc' || brand === 'hevx' || brand === 'heim' || brand === 'heis' || brand === 'hevm' || brand === 'hevs';
};
